<script lang="ts">
	...
	let { budget, onChangeValue } = $props<{
		budget: Budget;
		onChangeValue: (budget: Budget, value: number) => void | Promise<void>;
	}>();

	let formOpen = $state<boolean>(false);
	let valueInput = $state<HTMLInputElement | null>(null);
	let valueValue = $state<number>(budget.value);
	...
	async function handleEnter(event: KeyboardEvent): Promise<void> {
		onChangeValue(budget, valueValue);
		await toggleForm();
		keyManager.unregisterAll(keyHandlers);
	}
...
<div class="group flex w-full flex-row items-center justify-between">
	...
	{:else}
		<button class="cursor-text" ondblclick={() => toggleForm()}>
			{budget.type === 'income' ? '+ ' : '- '}{europeanFormat(budget.value)}
		</button>
	{/if}
</div>
